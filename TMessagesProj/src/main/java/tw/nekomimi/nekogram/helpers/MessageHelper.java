...
public class MessageHelper extends BaseController {
    ...
    public boolean sendMessageAsCopy(MessageObject messageObject, MessageObject.GroupedMessages messageGroup, long targetDialogId, MessageObject replyTo, MessageObject replyToTopMsg, ChatActivity.ReplyQuote quote, boolean notify, int scheduleDate, int mode, String quickReplyShortcut, int quickReplyShortcutId, long payStars, long monoForumPeerId, MessageSuggestionParams suggestionParams) {
        ...
        SendMessageChatArguments chatArguments = buildSendMessageChatArguments(mode, quickReplyShortcut, quickReplyShortcutId, targetDialogId);
        if ((messageObject.isSticker() || messageObject.isAnimatedSticker()) && messageObject.getDocument() != null) {
            SendMessagesHelper.getInstance(currentAccount).sendSticker(messageObject.getDocument(), null, targetDialogId, null, null, replyTo, replyToTopMsg, null, quote, null, notify, scheduleDate, 0, false, null, chatArguments, payStars, monoForumPeerId, suggestionParams);
            return true;
        }
        String path = getPathToMessage(messageObject, currentAccount);
        if (!TextUtils.isEmpty(path)) {
            ...
            if (messageObject.isRoundVideo()) {
                ...
                SendMessagesHelper.prepareSendingMedia(getAccountInstance(), media, targetDialogId, replyTo, replyToTopMsg, null, quote, false, false, null, notify, scheduleDate, 0, mode, false, null, chatArguments, 0, messageObject.messageOwner.invert_media, payStars, monoForumPeerId, suggestionParams);
                return true;
            } else if (messageObject.getDocument() != null) {
                ArrayList<String> paths = new ArrayList<>();
                paths.add(path);
                String mime = messageObject.getDocument().mime_type;
                SendMessagesHelper.prepareSendingDocuments(getAccountInstance(), paths, paths, null, caption != null ? caption.toString() : null, entities, mime, targetDialogId, replyTo, replyToTopMsg, null, quote, null, notify, scheduleDate, 0, null, chatArguments, 0, messageObject.messageOwner.invert_media, payStars, monoForumPeerId, suggestionParams);
                return true;
            }
        }
        if (caption != null && (messageObject.type == 0 || messageObject.isAnimatedEmoji()) && !TextUtils.isEmpty(caption)) {
            ...
            params.sendMessageChatArguments = chatArguments;
    ...
    private void flushSendingMedia(ArrayList<SendMessagesHelper.SendingMediaInfo> media, long targetDialogId, MessageObject replyTo, MessageObject replyToTopMsg, ChatActivity.ReplyQuote quote, boolean notify, int scheduleDate, int mode, String quickReplyShortcut, int quickReplyShortcutId, boolean invertMedia, long payStars, long monoForumPeerId, MessageSuggestionParams suggestionParams) {
        ...
        SendMessagesHelper.prepareSendingMedia(getAccountInstance(), media, targetDialogId, replyTo, replyToTopMsg, null, quote, false, media.size() > 1, null, notify, scheduleDate, 0, mode, false, null, buildSendMessageChatArguments(mode, quickReplyShortcut, quickReplyShortcutId, targetDialogId), 0, invertMedia, payStars, monoForumPeerId, suggestionParams);
    }

    private SendMessageChatArguments buildSendMessageChatArguments(int mode, String quickReplyShortcut, int quickReplyShortcutId, long targetDialogId) {
        SendMessageChatArguments.Builder builder = new SendMessageChatArguments.Builder();
        if (mode == ChatActivity.MODE_WELCOME_MESSAGES) {
            builder.setWelcomeMessageChatId(-targetDialogId);
        }
