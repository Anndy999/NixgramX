...
NIXGRAMX_VERSION_CODE=1267
```

Every APK that is **published** to either channel must use a never-before-published, strictly higher `NIXGRAMX_VERSION_CODE`. The previous Stable was `1266`, so this Stable promotion uses `1267`. This preserves Android upgrade paths in both directions.
