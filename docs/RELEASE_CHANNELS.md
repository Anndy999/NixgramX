...
NIXGRAMX_VERSION_CODE=1281
```

Every APK that is **published** to either channel must use a never-before-published, strictly higher `NIXGRAMX_VERSION_CODE`. The previous published build was `1275`, so this Beta uses `1281`. This preserves Android upgrade paths in both directions.
