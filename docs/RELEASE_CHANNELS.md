...
NIXGRAMX_VERSION_CODE=1273
```

Every APK that is **published** to either channel must use a never-before-published, strictly higher `NIXGRAMX_VERSION_CODE`. The previous Stable was `1272`, so this Stable promotion uses `1273`. This preserves Android upgrade paths in both directions.
